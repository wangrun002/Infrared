...
pj_version_name_list = ['SDK_V2.4.0', 'SDK_ecos_v2.3.0']            # 目标版本名称
...
class RedmineHandle(object):
    ...
    def get_specify_pj_version_id_by_name(self, project_id, version_name_list):
        version_id_dict = {}
        print('当前项目的所有版本信息如下：')
        print('{:#^50}'.format('开始'))
        for version in self.redmine.project.get(project_id).versions:
            ...
            for version_name in version_name_list:
                if version.name == version_name:
                    version_id_dict[version.id] = version.name
        print('{:#^50}'.format('结束'))
...
if __name__ == '__main__':
    ...
    # 创建输出目录
    now = datetime.now()
    pj_version_name_join = '+'.join(pj_version_name_list)
    output_dir = f'__{specify_project_name}__{filter_issue_subject_kws}__' \
                 f'{pj_version_name_join}__{now.strftime("%Y%m%d%H%M%S")}'
...
#    pj_version_id_list = RH.get_specify_pj_version_by_name(specify_project_id, pj_version_name)
    pj_version_id_dict = RH.get_specify_pj_version_id_by_name(specify_project_id, pj_version_name_list)
    print(pj_version_id_dict)

    # 筛选指定版本下的issue
    for pj_version_id in pj_version_id_dict:

        issues = RH.get_issues(specify_project_id, pj_version_id, 'open', 1)
        print(f'根据指定版本{pj_version_id_dict[pj_version_id]}过滤到的打开的错误个数为：{len(issues)}')
