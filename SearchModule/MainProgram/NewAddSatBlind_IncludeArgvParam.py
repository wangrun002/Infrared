...
UPPER_LIMIT_LATER_SEARCH_TIMES = 5       # 20 上限搜索后其他情况执行测试
...
DELETE_ALL_CH = [[KEY["MENU"],KEY["LEFT"],KEY["LEFT"],KEY["UP"],KEY["OK"],KEY["OK"]],[KEY["EXIT"]]]
...
class MyGlobal():
    def __init__(self):
        self.ser_cable_num = sys.argv[2]                # USB转串口线编号
        ...
        self.delay_save_channel_time = 20               # 用于搜索结束后的命令发送延时间隔
        ...
        self.all_sat_commd =   [
                                    ...
                                    [   ENTER_ANTENNA_SETTING, SEARCH_PREPARATORY_WORK[1],
                                        ["PLP D", "Polar=0", "5150/5750", "22K=0", "1", "0", "Blind"],
                                        CHOICE_BLIND_MODE, CHOICE_SAVE_TYPE[0],
                                        EXIT_ANTENNA_SETTING, NOT_OTHER_OPERATE,
                                        NORMAL_SEARCH_TIMES, NOT_UPPER_LIMIT_LATER_SEARCH_TIME],

                                    [   ENTER_ANTENNA_SETTING, SEARCH_PREPARATORY_WORK[1],
                                        ["PLP D", "Polar=0", "5150/5750", "22K=0", "1", "0", "SuperBlind"],
                                        CHOICE_SUPERBLIND_MODE, CHOICE_SAVE_TYPE[0],
                                        EXIT_ANTENNA_SETTING, NOT_OTHER_OPERATE,
                                        SUPER_SEARCH_TIMES, NOT_UPPER_LIMIT_LATER_SEARCH_TIME],
...
def delay_save_channel():
    ...
    if GL.delay_save_channel_time == 0:
        ...
        GL.delay_save_channel_time = 20  # 将该值恢复为初始值
        sys.exit(0)
    if GL.delay_save_channel_time > 0:
...
if len(GL.all_sat_commd[choice_search_sat]) < 9:
    ...
    search_time = GL.all_sat_commd[choice_search_sat][7]        # 选择执行轮次
...
write_xlsx_file_name = "{}_Sat_{}_SearchResult_{}.xlsx".format(sat_name,search_mode,timestamp)
...
while GL.MAIN_LOOP_STATE:
    ...
    if not data:
        ...
        # 执行多次运行的场景
            ...
            elif GL.switch_commd_stage == 7:
                if len(GL.all_sat_commd[choice_search_sat][6]) == 0:
                    logging.debug("Not Other Operate")
                    GL.switch_commd_stage += 1
                elif len(GL.all_sat_commd[choice_search_sat][6]) > 0:
                    ...
                    if not GL.upper_limit_state:
                        ...
                        if GL.all_sat_commd[choice_search_sat][6] == RESET_FACTORY:
                            ...
                            if GL.other_operate_sub_stage == 0:
                                GL.commd_global_length = len(GL.all_sat_commd[choice_search_sat][6])
                                if GL.commd_global_pos < GL.commd_global_length:
                                    GL.commd_single_length = len(GL.all_sat_commd[choice_search_sat][6][GL.commd_global_pos])
                                    if GL.commd_single_pos < GL.commd_single_length and GL.delay_delete_all_channels_state:
                                        del_all_chs_commd_data = GL.all_sat_commd[choice_search_sat][6][GL.commd_global_pos][GL.commd_single_pos]
                                        send_ser.write(hex_strs_to_bytes(del_all_chs_commd_data))
                                        # time.sleep(0.25)
                                        GL.commd_single_pos += 1
                                    elif GL.commd_single_pos == GL.commd_single_length:
                                        if GL.commd_global_pos + 1 < GL.commd_global_length:
                                            GL.commd_single_pos = 0
                                            GL.delay_delete_all_channels_state = False
                                            delay_delete_all_channels()
                                        GL.commd_global_pos += 1
                                elif GL.commd_global_pos == GL.commd_global_length:
            ...
            elif GL.switch_commd_stage == 8 and GL.delay_reset_factory_state:
    ...
    if data:
        ...
        if GL.search_monitor_kws[5] in data2:  # 监控保存TP的个数
            ...
            # GL.search_datas[0] = GL.all_sat_commd[choice_search_sat][1][-1]
