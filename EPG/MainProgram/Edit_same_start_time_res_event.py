...
# test_case_info = ['181', 'All', 'TV', 'Weekly', 'Play', 'TVScreenDiffCH', 'Manual_jump', 'Same[time+type]+Diff[mode+date]', 'Once', 'Play', 'screen_test_numb']
...
class MyGlobal(object):
    ...
    def __init__(self):
        ...
        self.cale_event_start_time = ['', '']           # 计算后的事件1和事件2的起始时间

        # 报告数据汇总[[预期事件1]，[预期事件2], [预约事件1]，[预约事件2], [预期编辑修改事件3],
        # "事件列表事件个数", "无效事件提示", "case编号", "执行case时间"]
        self.report_data = [[], [], [], [], [], '', '', '', '']
...
def create_log_and_report_file_path():
    ...
    case_name = "Edit_same_start_time_res_event"
    ...
    report_file_name = "Edit_same_start_time_res_event_result_report.xlsx"
...
def calculate_res_event_0_expected_start_time():
    logging.info("calculate_res_event_0_expected_start_time")
    time_interval_0 = 10
    time_interval_1 = 20
    str_expected_res_time_0 = ''
    str_expected_res_time_1 = ''
    ...
    if TEST_CASE_INFO[7] == "Same[time+type]+Diff[mode]" or TEST_CASE_INFO[7] == "Same[time]+Diff[type+mode]" \
        ...
        if TEST_CASE_INFO[3] == "Once" and TEST_CASE_INFO[8] in WEEKLY_EVENT_MODE:
            ...
            if cur_weekday == event_2_weekday:  # 当前系统时间对应的星期数与event_2对应的星期数相同时
                expected_res_time = dt_time + timedelta(minutes=time_interval_0)
                logging.info(expected_res_time)
                expected_res_time_split = re.split(r"[-\s:]", str(expected_res_time))
                str_expected_res_time_0 = ''.join(expected_res_time_split)[:12]
                ...
                if cur_weekday > event_2_weekday:
                    ...
                    cur_next_weekday_time = cur_next_weekday + timedelta(minutes=time_interval_0)
                    cur_next_weekday_time_split = re.split(r"[-\s:]", str(cur_next_weekday_time))
                    str_expected_res_time_0 = ''.join(cur_next_weekday_time_split)[:12]
    ...
    if TEST_CASE_INFO[7] == "Same[time+type]+Diff[mode]" or TEST_CASE_INFO[7] == "Same[time]+Diff[type+mode]" \
        ...
        if TEST_CASE_INFO[3] == "Once" and TEST_CASE_INFO[8] in WEEKLY_EVENT_MODE:
            ...
            event_2_weekday = weekday_num_dict[TEST_CASE_INFO[8]]   # event_2对应的星期数
            if cur_weekday == event_2_weekday:  # 当前系统时间对应的星期数与event_2对应的星期数相同时
                expected_res_time = dt_time + timedelta(minutes=time_interval_1)
                logging.info(expected_res_time)
                expected_res_time_split = re.split(r"[-\s:]", str(expected_res_time))
                str_expected_res_time_1 = ''.join(expected_res_time_split)[:12]
            elif cur_weekday != event_2_weekday:  # 当前系统时间对应的星期数与event_2对应的星期数不同时
                if cur_weekday > event_2_weekday:
                    interval_day = 7 - cur_weekday + event_2_weekday
                    cur_next_weekday = dt_time + timedelta(days=interval_day)
                    cur_next_weekday_time = cur_next_weekday + timedelta(minutes=time_interval_1)
                    cur_next_weekday_time_split = re.split(r"[-\s:]", str(cur_next_weekday_time))
                    str_expected_res_time_1 = ''.join(cur_next_weekday_time_split)[:12]
                elif cur_weekday < event_2_weekday:
                    interval_day = event_2_weekday - cur_weekday
                    cur_next_weekday = dt_time + timedelta(days=interval_day)
                    cur_next_weekday_time = cur_next_weekday + timedelta(minutes=time_interval_1)
    ...
    logging.info(f"期望的完整的预约事件时间为:event0:{str_expected_res_time_0},event1:{str_expected_res_time_1}")
    return str_expected_res_time_0, str_expected_res_time_1


def create_expected_add_event_0_info():
    logging.info("create_expected_add_event_0_info")
    # 创建期望的事件信息
    ...
    if TEST_CASE_INFO[7] == "Same[type+dur+mode]+Diff[time]" \
        ...
        duration_time = "0001"
    # 生成事件0和事件1的起始时间
    GL.cale_event_start_time = list(calculate_res_event_0_expected_start_time())

    if TEST_CASE_INFO[4] == "Play":
        expected_event_full_time = GL.cale_event_start_time[0]
...
def edit_add_new_res_event_0_info():
    logging.info("edit_add_new_res_event_0_info")
    # 编辑预约事件信息
    exit_to_screen = [KEY["EXIT"], KEY["EXIT"]]
    ...
    if TEST_CASE_INFO[3] == "Once":
        expected_res_event_info = create_expected_add_event_0_info()
    else:
        pretreatment_res_event_info = create_expected_add_event_0_info()    # 预处理预约事件信息
    ...
    send_cmd(KEY["OK"])
    # 添加新预约事件到事件管理列表
    GL.res_event_mgr.append(expected_res_event_info)
    # 添加新预约事件到report
...
def new_add_res_event_0():
    logging.info("new_add_res_event_0")
    # 新增预约事件
    enter_timer_setting_interface = [KEY["MENU"], KEY["LEFT"], KEY["DOWN"], KEY["OK"]]
    # 进入Timer_Setting界面
    send_more_cmds(enter_timer_setting_interface)
    ...
    # 进入事件编辑界面，设置预约事件参数
    edit_add_new_res_event_0_info()


def create_expected_add_event_1_info():
    logging.info("create_expected_add_event_1_info")
    # 创建期望的事件信息
    expected_event_info = ['', '', '', '', '']      # [起始时间，事件响应类型，节目名称，持续时间，事件触发模式]
    if TEST_CASE_INFO[7] == "Same[type+dur+mode]+Diff[time]" \
            or TEST_CASE_INFO[7] == "Same[type+dur]+Diff[time+mode]" \
        ...
        duration_time = "0001"
    if TEST_CASE_INFO[4] == "Play":
        expected_event_full_time = GL.cale_event_start_time[1]
        expected_event_info[0] = expected_event_full_time
        expected_event_info[1] = TEST_CASE_INFO[4]
        expected_event_info[2] = channel_info[1]
        expected_event_info[3] = "--:--"
...
def edit_add_new_res_event_1_info():
    logging.info("edit_add_new_res_event_1_info")
    # 编辑预约事件信息
    exit_to_screen = [KEY["EXIT"], KEY["EXIT"]]
    start_date_list = []        # 用于将开始日期由字符串转化为发送指令的列表
    start_time_list = []        # 用于将开始时间由字符串转化为发送指令的列表
    ...
    # 生成预期的预约事件
    if TEST_CASE_INFO[3] == "Once":
        expected_res_event_info = create_expected_add_event_1_info()
    else:
        pretreatment_res_event_info = create_expected_add_event_1_info()    # 预处理预约事件信息
        expected_res_event_info = pretreatment_res_event_info.copy()
        expected_res_event_info[0] = expected_res_event_info[0][8:]
    # 根据用例来编辑不同的事件
    # 检查是否进入到Timer Edit界面
    ...
    if TEST_CASE_INFO[4] == "PVR":      # 手动指定dur的‘：’间隔
        ...
        dur_time = new_expected_res_event_info[3]
        new_expected_res_event_info[3] = dur_time[:2] + ":" + dur_time[2:]
        GL.report_data[1].extend(new_expected_res_event_info)
    else:
        GL.report_data[1].extend(expected_res_event_info)
    # 退回大画面
    send_more_cmds(exit_to_screen)
...
def write_data_to_excel():
    ...
    wb = ''
    ws = ''
    excel_title_1 = ["用例编号", "预期事件1", "预期事件2", "保存事件1", "保存事件2", "预期编辑修改事件3", "编辑修改相同起始时间事件结果"]
    excel_title_2 = ["用例编号", "预期事件1", "预期事件2", "保存事件1", "保存事件2",
        ...
        if d in [0, 1, 2, 3]:
            ...
            if d == 2:  # 保存事件1
                ...
                if TEST_CASE_INFO[7] == "Same[mode]+Diff[time+type+dur]" \
                        ...
                        or TEST_CASE_INFO[7] == "Same[time+type]+Diff[mode+date]"\
                        or TEST_CASE_INFO[7] == "Same[time]+Diff[type+mode+date]":
                    if GL.report_data[d] != GL.report_data[1] and GL.report_data[d] == GL.report_data[4]:
                        ws.cell(max_row + 1, d + 2).font = dark_cyan_font    # 当保存事件2与预期事件2不相同
                    else:
                        ws.cell(max_row + 1, d + 2).font = red_font
                else:
                    if GL.report_data[d] == GL.report_data[1] and GL.report_data[d] != GL.report_data[4]:
                        ws.cell(max_row + 1, d + 2).font = blue_font    # 当保存事件2与预期事件2相同
                    else:
                        ws.cell(max_row + 1, d + 2).font = red_font

        elif d == 4:    # 预期编辑修改事件3信息
            ...
            if TEST_CASE_INFO[7] == "Same[mode]+Diff[time+type+dur]" \
                    ...
                    or TEST_CASE_INFO[7] == "Same[time+type]+Diff[mode+date]" \
                    or TEST_CASE_INFO[7] == "Same[time]+Diff[type+mode+date]":
                if GL.report_data[d] == 'Edit_same_start_time_res_event_success':
...
def edit_add_new_res_event_2_info():
    ...
    logging.info(GL.res_event_mgr)
    # 进入事件编辑界面
    logging.debug(GL.res_event_mgr == list(res_event_list))
    if GL.res_event_mgr == list(res_event_list):    # 当前两个事件的顺序与先后添加的事件顺序相同则需要发送down，否则就要编辑第一个
        send_cmd(KEY["DOWN"])   # 切到第二个事件上
    send_cmd(KEY["YELLOW"])
    # 生成预期的预约事件
    expected_res_event_info = create_expected_event_2_info()
    logging.info(expected_res_event_info)
    logging.info(TEST_CASE_INFO[8])
    ...
    if TEST_CASE_INFO[9] == "PVR":      # Invalid事件时，这里获取不到event_2的事件信息，所以只能手动指定dur的‘：’间隔
        ...
        dur_time = new_expected_res_event_info[3]
        new_expected_res_event_info[3] = dur_time[:2] + ":" + dur_time[2:]
        GL.report_data[4] = new_expected_res_event_info
    else:
        GL.report_data[4] = expected_res_event_info

    if rsv_kws["event_invalid_msg"] != '':
        GL.report_data[6] = rsv_kws["event_invalid_msg"]
        send_cmd(KEY["OK"])
    elif rsv_kws["event_invalid_msg"] == '':
        GL.report_data[6] = "Edit_same_start_time_res_event_success"
    # 退回大画面
    send_more_cmds(exit_to_screen)
...
def check_event_numb():
    ...
    while not state["res_event_numb_state"]:
        ...
        logging.info(rsv_kws["res_event_numb"])
        GL.report_data[5] = rsv_kws["res_event_numb"]
        state["res_event_numb_state"] = False
        # 获取预约事件的状态标志关闭
        state["update_event_list_state"] = False
        logging.info(list(res_event_list))
...
if __name__ == "__main__":
    ...
    while GL.res_triggered_numb > 0:
        clear_timer_setting_all_events()
        check_sys_time_mode()
        choice_ch_for_res_event_type("event_0")
        new_add_res_event_0()
        new_add_res_event_1()
