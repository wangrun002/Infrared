...
def choice_test_channel():
    if TEST_CASE[1] == "TV":
        if len(GL.TV_ch_attribute[3]) == 0:
            ...
            # GL.main_loop_state = False
            GL.current_stage += 1       # 假如没有合适的节目，则跳过执行下一步执行的动作，直接到退回大画面的步骤
            ...
            for i in range(len(GL.TV_ch_attribute[3])):
                ...
                if GL.channel_info[1] not in GL.all_ch_epg_info.keys():
                    GL.all_ch_epg_info[GL.channel_info[1]] = []         # 将有EPG信息的节目名称添加到字典
...
def padding_report_data():
    ...
    GL.actual_report_data[3] = REVERSE_KEY[SEND_TEST_CASE_COMMD[1]]
    if TEST_CASE[1] == "TV":
...
if __name__ == "__main__":
    ...
    TEST_CASE = ["GX", "TV", "Right", "SwitchEPGEvents"]
    TEST_CASE_COMMD = ["07", PREPARATORY_WORK, SEND_TEST_CASE_COMMD, EXIT_TO_SCREEN]
