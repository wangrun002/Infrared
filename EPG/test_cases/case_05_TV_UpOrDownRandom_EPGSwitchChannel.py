...
def send_test_case_commd():
    ...
    for i in range(GL.send_test_case_commd_numb):
        ...
        up_key_random_numb = randint(1,10)
        down_key_random_numb = randint(1,10)
        for j in range(up_key_random_numb):
            send_commd(send_data[0])
        for k in range(down_key_random_numb):
            send_commd(send_data[1])
        sleep_time = uniform(1.0, 2.0)
...
def padding_report_data():
    ...
    GL.actual_report_data[3] = "{} & {}".format(REVERSE_KEY[SEND_TEST_CASE_COMMD[0]],REVERSE_KEY[SEND_TEST_CASE_COMMD[1]])
...
def data_send_thread():
    ...
    #
    #     elif GL.current_stage == 1:  # 判断测试用例节目类型:TV/Radio
    #         check_ch_type()
    #
    #     elif GL.current_stage == 2:  # 判断是否有准备工作,比如进入某界面
    #         check_preparatory_work()
    #
    #     # elif GL.current_stage == 4:  # 选择并切到符合条件的节目
    #     #     choice_test_channel()
    #
    #     elif GL.current_stage == 3:  # 发送所选用例的操作指令,汇总报告数据，写报告数据
    #         send_test_case_commd()
    #         padding_report_data()
    #         write_data_to_report()
    #
    #     elif GL.current_stage == 4:  # 退回大画面
    #         send_data = TEST_CASE_COMMD[3]
    #         for i in range(len(send_data)):
    #             send_commd(send_data[i])
    #         GL.current_stage += 1
...
if __name__ == "__main__":
    ...
    SEND_TEST_CASE_COMMD = [KEY["UP"],KEY["DOWN"]]
    EXIT_TO_SCREEN = [KEY["EXIT"], KEY["EXIT"], KEY["EXIT"]]

    TEST_CASE = ["GX", "TV", "UpOrDownRandom", "EPGSwitchChannel"]
    TEST_CASE_COMMD = ["05", PREPARATORY_WORK, SEND_TEST_CASE_COMMD, EXIT_TO_SCREEN]
