...
def build_print_log_and_report_file_path():
    ...
    fmt_name = "{}_{}_{}_{}_{}".format(TEST_CASE_COMMD[0], TEST_CASE[0], TEST_CASE[1], TEST_CASE[2], TEST_CASE[4])
...
def get_group_channel_total_info():
    ...
    #             if GL.channel_info[3] == "1":
    #                 send_commd(KEY["EXIT"])
    #         if TEST_CASE[0] not in GL.TV_channel_groups.keys():
    #             logging.debug("不存在当前case指定的分组：{}，退出程序".format(TEST_CASE[0]))
    #             send_commd(KEY["EXIT"])
    #             GL.main_loop_state = False
    #         GL.sub_stage += 1
    ...
    #             if GL.channel_info[3] == "1":
    #                 send_commd(KEY["EXIT"])
    #         if TEST_CASE[0] not in GL.Radio_channel_groups.keys():
    #             logging.debug("不存在当前case指定的分组：{}，退出程序".format(TEST_CASE[0]))
    #             send_commd(KEY["EXIT"])
    #             GL.main_loop_state = False
    #         GL.sub_stage += 1
    ...
    for i in range(int(choice_group_ch_total_numb)):
        ...
        if TEST_CASE[1] == "TV":
            if GL.channel_info[3] == "1":  # 加锁电视节目
                ...
                if GL.channel_info[7] == "1": # 免费且有EPG信息的电视节目
            ...
            if GL.channel_info[3] == "1":  # 加锁广播节目
                ...
                if GL.channel_info[7] == "1": # 免费且有EPG信息的广播节目
    ...
    if TEST_CASE[1] == "TV":
        if "Free" == TEST_CASE[2]:
            if len(GL.TV_ch_attribute[3]) == 0:
                ...
                send_commd(KEY["OK"])
                time.sleep(2)
                logging.info("所选免费且有EPG信息的电视节目名称为:{}".format(GL.channel_info[1]))
                logging.info(GL.channel_info)
    elif TEST_CASE[1] == "Radio":
        if "Free" == TEST_CASE[2]:
            if len(GL.Radio_ch_attribute[3]) == 0:
                ...
                send_commd(KEY["OK"])
                time.sleep(2)
                logging.info("所选免费且有EPG信息的广播节目名称为:{}".format(GL.channel_info[1]))
                logging.info(GL.channel_info)
...
def send_test_case_commd():
    ...
    for i in range(GL.send_test_case_commd_numb):
        ...
        send_numb_key_commd(send_data[1])
...
def padding_report_data():
    # 期望的报告数据
    GL.expect_report_data[0] = "{}_{}_{}".format(TEST_CASE[1], TEST_CASE[2], TEST_CASE[4])
    ...
    GL.expect_report_data[5] = TEST_CASE[3]
    GL.expect_report_data[6] = GL.send_test_case_commd_numb

    # 实际的测试数据
    GL.actual_report_data[0] = GL.channel_info[6]
    GL.actual_report_data[2] = GL.channel_info[5]
    GL.actual_report_data[3] = GL.channel_info[1]
    GL.actual_report_data[4] = GL.channel_info[7]
    GL.actual_report_data[5] = GL.actual_test_numb
    if TEST_CASE[1] == "TV":
...
def data_send_thread():
    ...
    check_ch_type()             # 判断节目类型:TV/Radio
    check_preparatory_work()    # 判断是否有准备工作,比如进入某界面
    choice_test_channel()       # 选择并切到符合条件的节目
...
if __name__ == "__main__":
    ...
    SEND_TEST_CASE_COMMD = [KEY["EPG"], KEY["EXIT"]]
    EXIT_TO_SCREEN = [KEY["EXIT"], KEY["EXIT"], KEY["EXIT"]]

    TEST_CASE = ["GX", "Radio", "Free", "exist_epg_info", "EnterExitEpgInterface"]
    TEST_CASE_COMMD = ["02", PREPARATORY_WORK, SEND_TEST_CASE_COMMD, EXIT_TO_SCREEN]
