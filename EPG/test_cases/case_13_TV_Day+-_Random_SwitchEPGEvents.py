...
class MyGlobal():
    def __init__(self):
        ...
        self.each_ch_random_numb = 10                           # 每个有EPG信息的节目随机切换EPG事件的次数
...
def send_test_case_commd():
    ...
    while GL.send_commd_state:
        ...
        GL.each_ch_random_numb -= 1
        if GL.each_ch_random_numb == 0:
            GL.send_commd_state = False
            GL.each_ch_random_numb = 10
...
def padding_report_data():
    ...
    GL.actual_report_data[3] = "Day+OrDay-_Random"     # REVERSE_KEY[SEND_TEST_CASE_COMMD[1]]
    if TEST_CASE[1] == "TV":
...
if __name__ == "__main__":
    ...
    SEND_TEST_CASE_COMMD = [KEY["GREEN"], KEY["YELLOW"]]        # Green=Day-, Yellow=Day+
    EXIT_TO_SCREEN = [KEY["EXIT"], KEY["EXIT"], KEY["EXIT"]]

    TEST_CASE = ["GX", "TV", "Day+OrDay-_Random", "SwitchEPGEvents"]
    TEST_CASE_COMMD = ["13", PREPARATORY_WORK, SEND_TEST_CASE_COMMD, EXIT_TO_SCREEN]
