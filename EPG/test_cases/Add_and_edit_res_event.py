...
class MyGlobal(object):
    ...
    def __init__(self):
        ...
        # 报告数据汇总[[预约事件信息]，[修改后的预约事件信息], "系统时间日期", "触发时间", "等待节目", "跳转节目", "录制时长"]
        self.report_data = [[], [], '', '', '', '', '']
...
def write_data_to_excel():
    ...
    excel_title_1 = ["新增的预约事件信息", "编辑修改后的预约事件信息", "触发响应结果"]
    ...
    if not os.path.exists(file_path[1]):
        ...
        ws.cell(len(excel_title_0) + 1, 11).value = excel_title_1[2]
        ws["K" + str(len(excel_title_0) + 1)].alignment = alignment
        ws.merge_cells(start_row=len(excel_title_0) + 1, start_column=11, end_row=len(excel_title_0) + 1, end_column=15)
        # 写excel_title_2的内容
    ...
    len_total = len_res_1 + len_res_2 - 1       # 新增和编辑修改后的预约事件的总长度
    for d in range(len(GL.report_data)):
        if d == 0:      # 新增预约事件的信息
            for dd in range(len(GL.report_data[d])):
                ...
                ws.column_dimensions[get_column_letter(a_column_numb + dd)].width = 15
